...
//!
//! # relay-app — a crate whose layout was DERIVED from one declaration
//!
...
//! ## The module map
//!
//! * [`mixer`] (BOUNDARY) — value-object surface; `mixer_internal` (INTERIOR, private) —
//!   the workshop its operators delegate to, where the meaning holes live.
//! * [`gauge`] (BOUNDARY) — value-object surface; `gauge_internal` (INTERIOR, private) —
//!   the workshop its operators delegate to, where the meaning holes live.
...
//! 2. First build: `BLESS_RELAY_APP_QUALIFY=1 cargo build` — mints `spec/qualify.spec`, which the
