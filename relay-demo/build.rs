...
//!
//! * **The two censuses.** FIRST BUILD: run `BLESS_RELAY_APP_QUALIFY=1 BLESS_RELAY_APP_TIERS=1 cargo build`
...
fn main() {
    ...
    config.bless_env = "BLESS_RELAY_APP_QUALIFY".to_string();
    config.tiers_spec = Some(manifest.join("spec/tiers.spec"));
    config.tiers_bless_env = "BLESS_RELAY_APP_TIERS".to_string();
