...
### Autogenerated probing — the structure pays for itself (`src/laws.rs`)
The *structural* laws don't need to be written per edge: they follow from the
trait. `laws.rs` provides generic harnesses — `morphism_round_trips`,
`construction_round_trips`, `*_residual_complete`, and the `Pure ⇒ Unit`-residual
`capability_matches_residual` — each derived once from `Morphism`/`Construction`.
A new edge gets its whole structural suite by **registering it with an input
strategy (one line)**; no assertions are rewritten. So the boundary discipline
buys repeatable testing without hand-written tests, and `cargo-mutants` then
certifies those derived laws are strong (and surfaces any degree of freedom a
generated suite still misses — the `select`/`synth` loop). The *domain* laws that
do **not** follow from the algebra (commutativity, involution, the quantitative
coefficient) remain hand-written in `properties.rs`.
...
| Path | Role |
| --- | --- |
| `src/boundary.rs` | the grammar (a boundary as a **category**): sealed markers + edge shapes `Morphism` / `Construction` (entry edge) / `Branch` (coproduct) / `Guarded` (witnessed) + the grading `Monoid` (operator gradings residual=type / capability=const threaded by `Compose`; value provenance=type-level `Stamped`/`Step` lineage via `stamp_through`, reflected by `Lineage`) + probes `probe` / `commutes` / `coefficient_holds` / `reconstructs` / `construction_probe` / `Compose` + `Then` composition / retention typestate / `Qty` tagged primitives / `StateMachine`+`state_machine!`+`transition!` (carrier + descriptor + edges of a state graph) / `proof_token!` (zero-data name-branded proofs) / `Meter`+`Profiled` instrumentation seam |
...
| `src/laws.rs` | **autogenerated probing**: generic harnesses that derive each edge's structural laws (round-trip, residual completeness, `Pure ⇒ Unit` capability) from its trait — register an edge with a strategy and its suite is automatic |
| `src/properties.rs` | the probes + *domain* operator laws under `proptest` (the laws that don't follow from the algebra) |
