# algebra mutation: eager layout — 12 operator-table mutants, all killed — regenerate via this repo's freeze path; ratify the diff.
#
...
# output), judged by re-checking the discovered laws — 2 SURVIVED.
- SURVIVED  `render` goes deaf: always Placed([])
- SURVIVED  `reorder` goes deaf: always Placed([])
