...
mod tests {
    ...
    /// The RATIFIED residue, pinned exactly: four survivors, all of one deep kind —
    /// equational laws cannot state INEQUATIONS. The two action survivors are the TRIVIAL
    /// ACTION (`add`/`tick` returning their carrier unchanged satisfies action identity
    /// and the monoid-action law vacuously — "the action actually acts" is an inequation).
    /// `<` survives constant-false because irreflexivity (`x < x = false`) is its only
    /// named law and constant-false satisfies it — "somewhere `<` is true" is an
    /// inequation too. `diff` survives constant-zero because no named law pins it (an
    /// uncovered operator — the module distance already reports that silence). Closing
    /// any of these (a sharper shape, a declared expectation) must consciously edit this
    /// list.
    #[test]
    fn the_survivors_are_the_ratified_four() {
        assert_eq!(
            MutationReport::of::<Router>().survivors(),
            Vec::<&str>::new()
        );
        assert_eq!(
            MutationReport::of::<StoreProtocol>().survivors(),
            Vec::<&str>::new()
        );
        assert_eq!(
            MutationReport::of::<Arithmetic>().survivors(),
            vec!["`<` evaluates as `false`"]
        );
        assert_eq!(
            MutationReport::of::<Calendar>().survivors(),
            vec![
                "`diff` evaluates as `zero`",
                "`add` returns its first argument unchanged",
            ]
        );
        assert_eq!(
            MutationReport::of::<TtlStore>().survivors(),
            vec!["`tick` returns its first argument unchanged"]
        );
    }
