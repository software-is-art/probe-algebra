# algebra mutation: scale — 16 operator-table mutants, all killed — regenerate via this repo's freeze path; ratify the diff.
#
...
- killed    `plus` evaluates as `scale`
- killed    `neg` evaluates as `zero`
- killed    `scale` evaluates as `zero`
- killed    `scale` evaluates as `plus`
- killed    `scale` evaluates as `neg`
...
- killed    `scale` returns its first argument unchanged
- killed    `scale` returns its second argument unchanged
- killed    `zero` becomes undefined everywhere
- killed    `plus` becomes undefined everywhere
- killed    `neg` becomes undefined everywhere
- killed    `scale` becomes undefined everywhere
...
- SURVIVED  `scale` goes deaf: always []
