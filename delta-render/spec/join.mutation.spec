# algebra mutation: join — 16 operator-table mutants, all killed — regenerate via this repo's freeze path; ratify the diff.
#
...
- killed    `plus` evaluates as `join`
- killed    `neg` evaluates as `zero`
- killed    `join` evaluates as `zero`
- killed    `join` evaluates as `plus`
...
- SURVIVED  `join` goes deaf: always []
