# algebra mutation: sum — 15 operator-table mutants, 1 SURVIVED — regenerate via this repo's freeze path; ratify the diff.
#
...
- killed    `plus` evaluates as `sum`
- killed    `neg` evaluates as `zero`
- killed    `neg` evaluates as `sum`
- SURVIVED  `sum` evaluates as `zero`
- killed    `sum` evaluates as `neg`
