# discovered spec: sum — a behaviour lock; regenerate via this repo's freeze path and ratify the diff.
...
- neg leaves zero fixed.
      neg(zero) = zero
- neg and sum may be applied in either order.
      neg(sum(x)) = sum(neg(x))
