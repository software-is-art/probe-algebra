...
/// flags, for rules that predate `allowed_merge_methods`), and the private
/// vulnerability reporting flag. Unreadable payloads extract to the ABSENT state —
/// which the judge refuses by name, never assumes.
...
mod probes {
    ...
    fn the_api_payloads_extract_into_the_judged_facts() {
        ...
        let repo =
            r#"{"allow_merge_commit":true,"allow_squash_merge":true,"allow_rebase_merge":false}"#;
